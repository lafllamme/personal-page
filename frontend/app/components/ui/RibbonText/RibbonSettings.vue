...
<template>
  <div class="pointer-events-none absolute inset-0">
    <button
      v-if="panelHidden"
      class="bg-black/60 pointer-events-auto absolute right-4 top-24 z-30 border border-pureWhite/10 rounded-full px-3 py-1 text-[11px] color-pureBlack/80 dark:color-pureWhite/80 tracking-[0.2em] font-mono uppercase shadow-lg backdrop-blur-md"
    ...
    <div
      v-if="!panelHidden"
      class="bg-black/75 pointer-events-auto absolute right-4 top-24 z-30 max-h-[80vh] w-72 overflow-auto border border-pureWhite/10 rounded-lg p-3 text-[12px] color-pureBlack dark:color-pureWhite shadow-xl backdrop-blur-md space-y-2"
    >
      ...
      <div v-show="!panelCollapsed" class="space-y-2">
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ camX.toFixed(0) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Y</span>
          <input v-model.number="camY" type="range" min="-1000" max="1000" step="1">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ camY.toFixed(0) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Z</span>
          <input v-model.number="camZOffset" type="range" min="-2000" max="2000" step="5">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ camZOffset.toFixed(0) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Zoom</span>
          <input v-model.number="camZoom" type="range" min="0.25" max="3" step="0.01">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ camZoom.toFixed(2) }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ rotX.toFixed(2) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>RY</span>
          <input v-model.number="rotY" type="range" min="-3.14" max="3.14" step="0.01">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ rotY.toFixed(2) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>RZ</span>
          <input v-model.number="rotZ" type="range" min="-6.28" max="6.28" step="0.01">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ rotZ.toFixed(2) }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ scaler.toFixed(2) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Shift X</span>
          <input v-model.number="shiftX" type="range" min="-2000" max="2000" step="5">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ shiftX.toFixed(0) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Shift Y</span>
          <input v-model.number="shiftY" type="range" min="-2000" max="2000" step="5">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ shiftY.toFixed(0) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Shift Z</span>
          <input v-model.number="shiftZ" type="range" min="-2000" max="2000" step="5">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ shiftZ.toFixed(0) }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ lengthStretch.toFixed(2) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Width</span>
          <input v-model.number="widthStretch" type="range" min="0.5" max="3" step="0.01">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ widthStretch.toFixed(2) }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ textWeight }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <input v-model.number="textSize" type="range" min="24" max="140" step="1"
            @input="onDrawTextTexture?.()">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ textSize }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <input v-model.number="textStroke" type="range" min="0" max="22" step="1"
            @input="onDrawTextTexture?.()">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ textStroke }}</span>
        </label>

        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Width</span>
          <input v-model.number="textWidth" type="range" min="0.35" max="2.5" step="0.01">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ textWidth.toFixed(2) }}</span>
        </label>

        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Space</span>
          <input v-model.number="textSpacing" type="range" min="0" max="0.45" step="0.01">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ textSpacing.toFixed(2) }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ tracking }}</span>
        </label>

        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Height</span>
          <input v-model.number="typeHeight" type="range" min="0" max="80" step="1">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ typeHeight }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ textBoost.toFixed(2) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Mix</span>
          <input v-model.number="textMix" type="range" min="0" max="1" step="0.01">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ textMix.toFixed(2) }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ frameStrength.toFixed(2) }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ bandStrength.toFixed(2) }}</span>
        </label>
        ...
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ speed.toFixed(2) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          <span>Time</span>
          <input v-model.number="timeScale" type="range" min="0" max="2" step="0.05">
          <span class="text-[10px] color-pureBlack/60 dark:color-pureWhite/60 font-mono tabular-nums">{{ timeScale.toFixed(2) }}</span>
        </label>
        <label class="grid grid-cols-[auto,1fr] items-center gap-2">
          <span>Alt</span>
