...
<template>
  <div :class="useClsx(boxShadowClass, surfaceLight, surfaceDark, 'color-pureBlack w-full min-w-0 h-full overflow-hidden')">
      ...
      <div :class="useClsx('text-right min-w-0')">
        <div :class="useClsx('font-manrope mb-1 flex items-baseline justify-end gap-1 md:gap-1.5 leading-none')">
          ...
          <span
            ...
            :class="useClsx('ml-1 flex items-center gap-1 text-[clamp(12px,1.1vw,14px)]', stock.change < 0 ? 'color-crimson-10' : 'color-jade-9')"
          >
            <Icon :name="stock.change < 0 ? 'tabler:trending-down' : 'tabler:trending-up'" :class="useClsx('size-4')" />
            <span class="tabular-nums">{{ stock.change > 0 ? '+' : '' }}<NumberTicker :value="stock.change" /></span>
          </span>
        </div>
        <div :class="useClsx('font-manrope text-xs md:text-sm font-light color-gray-10 truncate')">{{ stock.industry || '—' }}</div>
      </div>
    </div>

    <div v-if="stock.dayRange.low !== null && stock.dayRange.high !== null" :class="useClsx('mt-4 border-t border-gray-7 border-solid pt-4')">
      ...
      <div :class="useClsx('relative')">
        ...
        <div v-if="stock.price !== null && stock.dayRange.low !== null && stock.dayRange.high !== null" :class="useClsx('absolute top-0 left-0 h-1 rounded-full from-olive-12 to-mint-11 bg-gradient-to-r transition-[width] duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]')" :style="{ width: rangeWidth }" />
